...
public class ResultAnalysis {
  Logger logger = LoggerFactory.getLogger(ResultAnalysis.class.getName());
  ...
  public void analysis(LevelAnalysisLevel level, ResultPicture currentPicture, Display display) {
    try {
      if (level == LevelAnalysisLevel.OVERVIEW) {
        display.displayLegend(
            List.of(currentPicture.timeOfPicture, "ZeebeSequence:L", "ZeebeDelta:L", "OperateSequence:L",
                "OperateDelta:L", "SequenceDelta:L", "Status:S", "ZeebeThroughput:S", "OperateThroughput:S"));
      }
      ...
      for (ResultComponent.TypeSequence typeSequence : ResultComponent.TypeSequence.values()) {
        ...
        if (level == LevelAnalysisLevel.FULL) {
          display.displayLegend(
              List.of(typeSequence.toString() + ":S", "partition:I", "ZeebeSequence:L", "ZeebeDelta:L",
                  "OperateSequence:L", "OperateDelta:L", "SequenceDelta:L", "Status:S", "ZeebeThroughput:S",
                  "OperateThroughput:S"));
        }
        ...
        for (int i = 1; i <= zeebeCurrent.getMaxPartitionsId(); i++) {
          ...
          if (level == LevelAnalysisLevel.FULL) {
            ...
            list.add(
                getThroughputPerMin(previousPicture != null ? previousPicture.timeOfPicture : null, // previous time
                    zeebePreviousSequence, // previous value
                    currentPicture.timeOfPicture, // current value
                    zeebeCurrentSequence // current value
                ));
            list.add(getThroughputPerMin(previousPicture != null ? previousPicture.timeOfPicture : null,
                zeebePreviousSequence, currentPicture.timeOfPicture, operateCurrentSequence));
        ...
        if (level == LevelAnalysisLevel.OVERVIEW) {
          List<Object> list = new ArrayList();
          list.add(typeSequence.toString() + ":D");
  ...
  private String getThroughputPerMin(LocalDateTime previousTime,
    ...
    if (previousTime == null || currentTime == null || currentValue == null || previousValue == null)
      return "- rec/mn";
